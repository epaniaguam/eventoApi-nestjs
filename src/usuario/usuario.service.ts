...
export class UsuarioService {
  ...
  async updateByName(usrnm: string, updateUsuarioDto: UpdateUsuarioDto) {
    ...
    if (usuarioIgual) {
      const existe = await this.usuarioRepository.findOne({
        where: { username: usuarioIgual },
      });
      if (existe) {
        throw new HttpException(
          {
            message: `Otro usuario con ${updateUsuarioDto.username} ya existe`,
          },
          HttpStatus.CONFLICT,
        );
      }
    ...
    const usuarioActualizado = Object.assign(existeUsuario, updateUsuarioDto);
